		<select class="select_mutation_aa chosen-300" name="select_mutation_aa" data-placeholder="Select Mutation - AA">
			<option value=""></option>
			<?php
				$sql = "SELECT DISTINCT aa_location, aa_original_id, aa_substituted_id, codon_original_id, codon_substituted_id FROM paper_mutations WHERE id IN (SELECT mutation_id FROM view_dgid_paper_mutations WHERE drug_gene_id IN (SELECT drug_gene_id FROM paper_drug_gene WHERE id=?)) ORDER BY aa_location";
				$q = $conn -> prepare($sql);
