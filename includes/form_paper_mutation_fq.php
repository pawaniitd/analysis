...
<form class="form_paper_mutation_fq">
	...
	<select class="select_gene_id" name="paper_gene_id" data-placeholder="Choose a Gene... " required>
		...
		<?php
			try {
			$sql = "SELECT id, name FROM h37rv_genes WHERE id IN (SELECT DISTINCT gene_id FROM drug_gene WHERE drug_id IN (SELECT id FROM drugs WHERE category='Fluoroquinolones'))";
			$q = $conn -> prepare($sql);
			$q->execute();
